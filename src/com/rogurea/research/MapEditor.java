...
public class MapEditor {
    ...
    static char EmptyCell = Resources.EmptyCell;
    ...
    static void FillAllSpaceWithEmpty(char[][] ShapeBuffer){
        for (int i = 0; i < ShapeBuffer.length; i++) {
            for (int j = 0; j < ShapeBuffer[0].length; j++) {
                    ShapeBuffer[i][j] = EmptyCell;
            }
...
/*                System.out.println(
    ...
    static void PlaceDoors(R_Room room, char[][] CurrentRoom){
        if(!room.IsEndRoom)
            CurrentRoom[GenerateRules.ye][GenerateRules.xe] = Symbols.ARROW_DOWN;
