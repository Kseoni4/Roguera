...
/*        if(y0 == 0 && x0 == 0)
   ...
   /* public static void PlacePoints(char[][] CurrentRoom) {

    ye = y0; xe = x0;
    yp = y0; xp = x0+(CurrentRoom[0].length/2);
    ConnectPoints(yp, xp, CurrentRoom, true);
    while(y0 >= 0 && x0 >= 0){
        OuterPlace(CurrentRoom);
        SearchXY(x0, y0, yp, xp, CurrentRoom);
        System.out.printf("\nO_XY[%d;%d]\n", y_, x_);
        ConnectPoints(y_, x_, CurrentRoom, true);
        ConnectPoints(yp, xp, CurrentRoom, true);
        p++;
        if(xp > CurrentRoom[0].length || xp < 0)
            break;
    }
    System.out.println("y0: "+ y0 + " " + "x0: " + x0);
    System.out.println("yp: "+ yp + " " + "xp: " + xp);

    yp = 0; xp = (CurrentRoom[0].length-1);
    ...
    System.out.printf("\nE_XY[%d;%d]\n", y_, x_);
    ConnectPoints(yp, xp, CurrentRoom, true);
    ConnectPoints(0, 0, CurrentRoom, true);

}   public static void ConnectPoints(int y, int x, char[][] CurrentRoom, boolean IsReverse){
        ChangeDirection(y, x);
        if(lastDrawDirection == MapEditor.DrawDirection.LEFT)
        IsReverse = !IsReverse;
        if(IsNotOutOfBoundsRule(y,x, CurrentRoom)) {

            IsReverse = !IsReverse;

            CurrentRoom[y][x] = drawDirection.getCorner(IsReverse);

            MapEditor.InsertShapeLine(CurrentRoom, drawDirection, l, x0, y0);

            IsReverse = !IsReverse;

            CurrentRoom[y0][x0] = drawDirection.getCorner(IsReverse);

            System.out.println("Point line from [" + y0 + ";" + x0 + "]");
    ...
    private static boolean IsInIntervalRule(int n){
        if(Math.max(xp,xp_) - Math.min(xp,xp_) >= n){
            return true;
        }
        if(Math.max(yp,yp_) - Math.min(yp,yp_) >= n){
            return true;
        }
        return false;
    }

    static void SearchXY(int x0, int y0, int yp, int xp, char[][] CurrentRoom){
        int xp_s;
        int yp_s;
        ...
        if(yp>xp){
            ...
            x_ = xp+xp_s;
            if(IsNotOutOfBoundsRule(y_, x_, CurrentRoom)){
                CurrentRoom[y0+yp_s][xp+xp_s] = String.valueOf(p).toCharArray()[0];
                System.out.printf("\nSetPointin[%d;%d]", y_, x_);
            }
        }
        else if (xp>yp){
            yp_s = y0 - yp;
            ...
            x_ = x0+xp_s;
            if(IsNotOutOfBoundsRule(y_, x_, CurrentRoom)) {
                CurrentRoom[yp + yp_s][x0 + xp_s] = String.valueOf(p).toCharArray()[0];
                System.out.printf("\nSetPointin[%d;%d]", y_, x_);
            }
        }
